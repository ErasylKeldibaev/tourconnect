...
class TravelProvider with ChangeNotifier {
  ...
  List<Place> get favoritePlaces =>
      places.where((p) => _favoritePlaceIds.contains(p.id)).toList();

  // Избранные туры
  List<Tour> get favoriteTours =>
      tours.where((t) => _favoriteTourIds.contains(t.id)).toList();

  // =========================
  // ГОРОДА
  // =========================

  bool isCityFavorite(String id) {
    return _favoriteCityIds.contains(id);
  }
  ...
  bool isPlaceFavorite(String id) {
    return _favoritePlaceIds.contains(id);
  }
  ...
  bool isTourFavorite(String id) {
    return _favoriteTourIds.contains(id);
  }
  ...
  List<Place> getPlanForCity(String cityId) {
    return _cityTripPlans[cityId] ?? [];
  }

  bool isPlaceInPlan(String cityId, String placeId) {
    return (_cityTripPlans[cityId] ?? []).any(
          (p) => p.id == placeId,
    );
  }
